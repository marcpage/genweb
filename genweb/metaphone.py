...
def dm(st):
    ...
    while pos <= last:
        ...
        if ch in vowels:
            ...
            # double 'C', but not if e.g. 'McClellan'
            elif st[pos : pos + 2] == "CC" and not (
                pos == (first + 1) and st[first] == "M"
            ):
                #'bellocchio' but not 'bacchus'
                if st[pos + 2] in ["I", "E", "H"] and st[pos + 2 : pos + 4] != "HU":
                    #'accident', 'accede' 'succeed'
                    if (pos == (first + 1) and st[first] == "A") or st[
                        pos - 1 : pos + 4
                    ] in ["UCCEE", "UCCES"]:
                        nxt = ("KS", 3)
                ...
                if st[pos + 1 : pos + 3] in [" C", " Q", " G"]:
                    ...
                    if st[pos + 1] in ["C", "K", "Q"] and st[pos + 1 : pos + 3] not in [
                        "CE",
                        "CI",
                    ]:
                        nxt = ("K", 2)
            ...
            if st[pos + 1] == "H":
                ...
                # Parker's rule (with some further refinements) - e.g., 'hugh'
                    ...
                    (pos > (first + 1) and st[pos - 2] in ["B", "H", "D"])
            ...
            # -ger-,  -gy-
                ...
                (st[pos + 1 : pos + 2] == "ER" or st[pos + 1] == "Y")
                ...
                nxt = ("K", "J", 2)
            # italian e.g, 'biaggi'
            elif st[pos + 1] in ["E", "I", "Y"] or st[pos - 1 : pos + 3] in [
                "AGGI",
                "OGGI",
            ]:
                # obvious germanic
                if (
                    st[first : first + 4] in ["VON ", "VAN "]
                    or st[first : first + 3] == "SCH"
            ...
            if st[pos : pos + 4] == "JOSE" or st[first : first + 4] == "SAN ":
                if (pos == first and st[pos + 4] == " ") or st[
                    first : first + 4
                ] == "SAN ":
                    nxt = ("H",)
                else:
            ...
            if st[pos + 1] == "L":
                ...
                if (
                    ...
                    (st[last - 1 : last + 1] in ["AS", "OS"] or st[last] in ["A", "O"])
            ...
            elif (pos == first and st[pos + 1] in ["M", "N", "L", "W"]) or st[
                pos + 1
            ] == "Z":
                nxt = ("S", "X")
                if st[pos + 1] == "Z":
            ...
            # Arnow should match Arnoff
                ...
                (pos == last and st[pos - 1] in vowels)
...
if __name__ == "__main__":
    ...
    for name in names.keys():
        if dm(name) == names[name]:
            print("name = ", name, "   Metaphone = ", dm(name)[0], ",", dm(name)[1])
            # print('For ',name,'function returned ',dm(name),'. Should be ',names[name]))
